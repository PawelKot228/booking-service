...
@if(\App\Enums\AppointmentStatus::REJECTED->value === $status)
...
@elseif(\App\Enums\AppointmentStatus::FINISHED->value === $status)
    <span class="bg-teal-100 text-teal-800 border-teal-300 border-2 rounded-md text-xs font-medium mr-2 px-2.5 py-0.5 dark:bg-teal-900 dark:border-teal-500 dark:text-teal-300">
        {{ __('Finished') }}
    </span>
@else
